...
public class iceBlock_2 : MonoBehaviour {

	private const int NUM_ROWS = 7;
	private const int NUM_COLS = 7;
	public playerArrowIce_2 player;
	public iceBlock_2[] otherBlocks;
	...
	void Start () {
		...
		*/
		...
		for(int y = 4; y < 7; y++) {
			movementSquares[i] = "" + 2 + "" + y;
			i++;
		}

		for(int y = 3; y < 7; y++) {
			movementSquares[i] = "" + 3 + "" + y;
			i++;
		}

		for(int y = 2; y < 7; y++) {
			movementSquares[i] = "" + 4 + "" + y;
			i++;
		}

		for(int y = 3; y < 7; y++) {
			movementSquares[i] = "" + 5 + "" + y;
			i++;
		}

		for(int y = 3; y < 6; y++) {
			movementSquares[i] = "" + 6 + "" + y;
			i++;
		}
	}
	...
	private bool blockedByIce() {
		if(predictedSquare == otherBlocks[0].square) {
			return true;
		} else {
			return false;
		}
